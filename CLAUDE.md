...
**The hover reports what it actually landed on** (`go_to_hover`, 2026-08-07). `poses.goto` walks every joint in sub-cap hops and re-checks the travel limits *per hop*, so a joint that is out of range — back-driven by a power cut, left there by a stalled descent — is refused part-way while the others arrive, and the run then continues from a pose that **looks like the hover in the log because the move was commanded** and is not one. It prints the per-joint residual and names any joint outside `SERVO_VISUAL_HOVER_TOLERANCE_TICKS` (15) with the limits that refused it. It cannot ask about that any more, so it has to say it — and the go-ahead immediately below is the moment to act on it.
...
**The run asks TWO questions and no more** (2026-08-07, operator's call): the go-ahead (`g`) and the further-close prompt at the grip. The claw is opened automatically at the hover, closed automatically at the bottom. Removed: the descend confirmation, the "close the claw now?" offer, the per-jog approvals, the "did the gap shrink?" ruler check (J5's `dir_sign` was the reason for that one and it is now jog-confirmed, and every step already reports FK's commanded-vs-actual), and the "drive to hover again?" prompt.

**The claw opens at the HOVER, not at the bottom.** The jaws swing ~490 ticks and there is room for that at the top and none beside a brick a few millimetres above the table. A run that arrives at grasp height with the claw already shut has also spent the whole descent unable to do the one thing it came for.

**`gripper.auto_close` uses the servo as its own contact sensor**, which is a better instrument than the operator's eye for this one question. Two phases asking different things of the same measurement:

- **SEARCH** — close in `SERVO_GRIPPER_AUTO_CLOSE_TICKS` (25) steps. A step delivering less than `SERVO_GRIPPER_CONTACT_TICKS` (8) has met something. Compared against `min(step, asked)` so the last, clamped step cannot register as false contact.
- **FIRM UP** — squeeze from contact until `SERVO_GRIPPER_FIRM_STEPS` (2) consecutive squeezes deliver nothing. The first squeezes often still move as the jaws seat and the brick settles; a run of quiet ones is the servo holding position error, which is force.

**The search floor is the measured grip position (3003), not the full-close stop.** Contact should happen at or before it, so reaching it with the jaws still moving freely means nothing is between them — and closing further on nothing is the case the operator named as "it should never be this much". That is reported as a miss and the claw stops there.
...
`two_view_survey` runs **after centring and before the descent** — the only window worth anything: the brick is centred and in view, the claw is still high enough that a sideways swing is free, and no height has been committed to. The in-descent pairs give the same measurement for free afterwards, but only in arrears.

**LEFT, back to centre, RIGHT, back to centre** (operator-specified 2026-08-07), each leg `SERVO_VISUAL_TWO_VIEW_BASELINE_MM` (24 mm) **tangentially**. That is a half-baseline: the two views end up **48 mm apart, ~13° of parallax**, while each individual *move* is only 24 mm and therefore still fits the tangential pan budget. A single 48 mm move would not — it needs 13.4° of base yaw against `SERVO_VISUAL_MAX_TANGENTIAL_PAN_DEG` of 8, so it would be halved and the baseline lost. **Symmetry buys the parallax the budget would otherwise refuse.** Returning through centre between the poses is not decoration either: it re-reads the arm from the pose the descent will actually start from, so a move the budget shrank shows up as a gap to close rather than as drift nobody measured (the offset is tracked absolutely, not as a running total of deltas).
